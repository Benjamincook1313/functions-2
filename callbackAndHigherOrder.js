...
let uniq = (arr, cb) => {
  ...
  for(let i=0; i<arr.length; i++){
    if(!noDups.includes(arr[i])) noDups.push(arr[i])
  }
  cb(noDups)
};
...
uniq(names, uniqArr => console.log(`The new names array with all the dupicate items is [ ${uniqArr} ]`))
////////// PROBLEM 6 //////////
...
const getUserById = (arr, id, cb) => {
  let userInfo = arr.filter(user => user.id === id)
  cb(userInfo[0])
};
